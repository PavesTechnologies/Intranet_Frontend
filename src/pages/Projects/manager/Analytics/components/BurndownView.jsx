...
const BurndownView = ({
  ...
  const meta = {
    title:       `Sprint Burndown — ${sprintName ?? "Sprint"}`,
    subtitle:    [startDate, endDate].filter(Boolean).join(" – "),
    legendItems: LEGEND_ITEMS_BURNDOWN,
  };

  const handlePNG = () =>
    downloadChartWithScopeAsPNG(chartRef.current?.getCanvas(), scopeChanges, `${sprintName}_Burndown`, meta);
  const handlePDF = () =>
    downloadChartWithScopeAsPDF(chartRef.current?.getCanvas(), scopeChanges, `${sprintName}_Burndown`, meta);
  const handleCSV = () => {
