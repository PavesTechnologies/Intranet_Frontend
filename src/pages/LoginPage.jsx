...
.login-page-container .btn-ms {
  width:100%; height:48px; background:#2F2F2F; color:#fff; border:none; border-radius:6px;
  display:flex; align-items:center; justify-content:center; gap:10px;
  font-size:14px; font-weight:500; font-family:var(--ff); letter-spacing:.005em;
  cursor:pointer;
...
.login-page-container .field-input:-webkit-autofill ~ .field-label {
...
.login-page-container .field-input:-webkit-autofill {
  padding-top:18px;
}
...
@media (max-width:820px) {
  .login-page-container .left-panel { display:none !important; }
  .login-page-container .right-panel { min-height:100vh; }
}
@media (prefers-reduced-motion: reduce) {
  .login-page-container *, .login-page-container *::before, .login-page-container *::after { animation-duration:1ms !important; transition-duration:1ms !important; }
}
`;
    ...
    const draw = () => {
      ...
      for (let i = 0; i < dots.length; i++) {
        ...
        if (dist < maxDist) {
          ...
          // Interpolate color from light grey (#E7E5E4) to darker grey (#78716C)
...
export default function LoginPage() {
  ...
  const EYE_D = showPw
    ...
    : 'M13.5 7s-2 3.5-5.5 3.5S2.5 7 2.5 7 4.5 3.5 8 3.5 13.5 7 13.5 7zM8 9a2 2 0 1 1 0-4 2 2 0 0 1 0 4z';
      ...
      >
        ...
        <div style={{ position: 'relative', zIndex: 1, flex: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {/* Watermark */}
            <div style={{ fontSize: '84px', fontWeight: 700, lineHeight: .88, color: '#d8d3c8ff', letterSpacing: '-0.05em', marginBottom: '40px', userSelect: 'none', pointerEvents: 'none' }}>
              Work<br />Space
            </div>

            <h1 style={{ fontSize: '24px', fontWeight: 700, color: 'var(--neutral-950)', letterSpacing: '-0.022em', lineHeight: 1.25, marginBottom: '14px' }}>
              Your organization's<br />operational hub
            </h1>

            <p style={{ fontSize: '14px', fontWeight: 400, color: 'var(--neutral-500)', lineHeight: 1.7, maxWidth: '550px', marginBottom: '48px' }}>
              Manage leave, track timesheets, handle projects, and access all your internal tools through a single gateway.
            </p>

            {/* Services */}
            <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
              {SERVICES.map(({ label, d }) => (
                <div key={label} className='svc-item' style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '7px', opacity: .55 }}>
          ...
          {/* <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: 'auto' }}>
            <svg width={12} height={12} viewBox='0 0 16 16' fill='none' stroke='var(--neutral-400)' strokeWidth={1.5} aria-hidden='true'>
              <circle cx={8} cy={8} r={6} /><path d='M8 7v4M8 5.5v.5' strokeLinecap='round' />
            </svg>
            <span style={{ fontSize: '12px', color: 'var(--neutral-400)' }}>
            Need help? Contact IT at <span style={{ color: 'var(--neutral-500)', fontWeight: 500 }}>ext. 100</span>
          </span>
          </div> */}
        </div>
      </div>
      ...
      >
        ...
        <div style={{ position: 'absolute', bottom: '28px', display: 'flex', alignItems: 'center', gap: '14px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
            <svg width={10} height={10} viewBox='0 0 16 16' fill='none' stroke='var(--neutral-300)' strokeWidth={1.5} aria-hidden='true'>
