...
const CategoryDetailDrawer = ({ open, record, loading, onClose, onToggle, onEdit }) => {
  ...
  return (
    ...
    >
      <div
        className={`h-full w-full max-w-2xl border-l border-slate-200 bg-white shadow-2xl transition-transform duration-300 ${open ? "translate-x-0" : "translate-x-full"}`}
        onClick={(event) => event.stopPropagation()}
      >
        <div className="flex h-full flex-col">
          <div className="flex items-center justify-between border-b border-slate-200 px-6 py-5">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Asset Category</p>
              <h2 className="mt-1 text-2xl font-semibold text-slate-900">{record.categoryName || "-"}</h2>
              <div className="mt-2 grid gap-2 sm:grid-cols-2">
                ...
                <div className="inline-flex items-center gap-2 text-xs">
                  ...
                  <span className="text-slate-600">{record.activeFlag ? "Active" : "Inactive"}</span>
                </div>
              </div>
            </div>

            <div className="flex items-center gap-3">
              <Button variant="primary" onClick={() => onEdit && onEdit(record)}>Edit</Button>
              <Button
                variant={record.activeFlag ? "danger" : "success"}
                onClick={() => onToggle && onToggle(record)}
                loading={loading}
              >
                {record.activeFlag ? "Deactivate" : "Activate"}
              </Button>
              <button
                type="button"
                onClick={onClose}
          ...
          <div className="flex-1 overflow-y-auto px-6 py-6">
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
                <div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-500">Code</div>
                <div className="text-sm font-medium text-slate-700">{record.categoryCode ?? "-"}</div>
              </div>

              <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
                <div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-500">Asset Nature</div>
                <div className="text-sm font-medium text-slate-700">{record.assetNature || record.nature || "-"}</div>
              </div>

              <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
                <div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-500">Serial Number Required</div>
                <div className="text-sm font-medium text-slate-700">
                  {record.requiresSerialNumber || record.serialNumberRequired ? "Yes" : "No"}
                </div>
              </div>

              <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
                <div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-500">Name</div>
                <div className="text-sm font-medium text-slate-700">{record.categoryName ?? "-"}</div>
              </div>
            </div>
...
const AssetCategoryManagement = () => {
  ...
  const rows = useMemo(() => {
    const q = (filters.categoryCode || filters.categoryName || "").toLowerCase().trim();
    ...
    const ordered = q ? [...categories].sort((a, b) => scoreItem(b) - scoreItem(a)) : categories;

    return ordered.map((category) => ({
      ...category,
      code_info: (
        <span className="font-semibold text-gray-900">{category.categoryCode}</span>
      ),
      name_info: (
        <span className="font-medium text-gray-800">{category.categoryName}</span>
      ),
      nature_info: (
        <span className="text-sm text-slate-600">{category.assetNature || category.nature || "IT"}</span>
      ),
      serial_info: (
        <span className="text-sm text-slate-600">
          {(category.requiresSerialNumber || category.serialNumberRequired) ? "Yes" : "No"}
        </span>
      ),
  ...
  }, [categories, actionLoadingId, filters.categoryCode, filters.categoryName]);
        ...
        <div className="rounded-2xl border border-gray-200 bg-white shadow-sm">
          <div className="flex flex-col gap-4 border-b border-gray-100 bg-gray-50/30 p-5 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <SearchInput
                value={filters.categoryCode || filters.categoryName}
                onChange={(value) => {
                  // Use single search input for both code and name
                  setFilters((prev) => ({ ...prev, categoryCode: value, categoryName: value }));
                  setPageInfo((prev) => ({ ...prev, current: 0 }));
                }}
          ...
          <div className="overflow-x-auto p-4">
            {error && !loading ? (
              ...
              <GenericTable
                ...
                columns={["code_info", "name_info", "nature_info", "serial_info", "status_info", "actions"]}
                rows={rows}
              />
            )}
          </div>
